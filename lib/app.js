...
var persons_result_map = { done: false }
, works_result_map = { done: false }
, expressions_result_map = { done: false }
, manifestations_result_map = { done: false }
...
module.exports = {
  ...
  search: function (query, callback) {
    ...
    var test = function () {
      callback({
        persons: persons_result_map,
        works: works_result_map,
        expressions: expressions_result_map,
        manifestations: manifestations_result_map
      })
    }

    queries.person.on('end', function (data) {
      persons_result_map.done = true
      is_done(test)
    })
    queries.work.on('end', function (data) {
      works_result_map.done = true
      is_done(test)
    })
    queries.expression.on('end', function (data) {
      expressions_result_map.done = true
      is_done(test)
    })
    queries.manifestation.on('end', function (data) {
      manifestations_result_map.done = true
      is_done(test)
    })
