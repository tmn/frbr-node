...
var do_i_exist = function (list, item) {
...
var build_rels = function (root, item) {
  if (do_i_exist(root.children, item) || do_i_exist(item.children, root)) {
    return
  }

  if (item.name) root.children.push(item)
  ...
  if (item.doc !== undefined && item.doc.relationship.length > 0) {
    item.doc.relationship.forEach(function (rel) {
      search_res.forEach(function (res) {
        if (res[rel['f:href']]) {
          build_rels(item, res[rel['f:href']])
        }
      })
    })
  }
...
var make_tree = function (root, data) {
  for (var key in data) {
